...
TEST_CASE("A quote that is never closed is refused", "[cfg_file]") {
    ...
    CHECK_THROWS_WITH(
        parsed("origin \"A BBS"),
        Catch::Matchers::Contains("t.cfg:1: a quoted value is never closed"));
}

TEST_CASE("What a toml config left behind is named for what it is", "[cfg_file]") {
    CHECK_THROWS_WITH(parsed("[general]\ntosser_config a"),
                      Catch::Matchers::Contains("[section]"));
    CHECK_THROWS_WITH(parsed("tosser_config = \"a\""),
...
TEST_CASE("An entry says which line it came from", "[cfg_file]") {
    ...
    CHECK_THROWS_WITH(entries.front().fail("no good"),
                      Catch::Matchers::Contains("t.cfg:3: no good"));
}
...
TEST_CASE("Values are read as the kind the setting wants", "[cfg_file]") {
    ...
    CHECK_THROWS_WITH(only("quote_margin 7x").number(),
                      Catch::Matchers::Contains("whole number"));
    CHECK_THROWS_WITH(only("quote_margin 300").numberIn(20, 255),
                      Catch::Matchers::Contains("between 20 and 255"));
    CHECK_THROWS_WITH(only("back_button 1").flag(),
                      Catch::Matchers::Contains("on or off"));
    CHECK_THROWS_WITH(only("quote_margin").number(),
                      Catch::Matchers::Contains("exactly one value"));
    CHECK_THROWS_WITH(only("quote_margin 20 30").number(),
                      Catch::Matchers::Contains("exactly one value"));
    CHECK_THROWS_WITH(only("name").text(), Catch::Matchers::Contains("needs a value"));
}
