...
TEST_CASE("An external utility is asked for from the area list [arealist][keys]") {
    ...
    // key leaves behind is the command for `runApp()` to answer on the next
    // pass.
    ...
    CHECK(*fixture.state.externUtilRequested ==
          amberedit::config::Command::AreaListExternUtil2);
...
/// Turns the weight on the selection bar on for as long as it stands — what a
/// theme with `selection_bold on` in it does.
struct SelectionBoldOn {
    SelectionBoldOn() { amberedit::ui::theme::palette.selectionBold = true; }
    ~SelectionBoldOn() { amberedit::ui::theme::palette.selectionBold = was; }
...
TEST_CASE("selection_bold adds weight to the bar and changes nothing else "
    ...
    // The switch is off unless a theme asks for it: the bar stands in its two
    // colors, and which row Enter would act on is said by the fill alone.
    term::Screen screen(fixture.state.width, fixture.state.height);
    term::render(screen, area_list::render(fixture.state));
    ...
    // On, the weight is added and nothing else is: the bar keeps both colors.
    const SelectionBoldOn heavy;
    term::render(screen, area_list::render(fixture.state));
