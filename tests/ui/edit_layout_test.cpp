...
TEST_CASE("A line wider than the window is shown over several rows [editlayout]") {
    ...
    // Seven columns of text in a window of eight: the eighth is the cursor's.
    ...
    CHECK(shownRows(lines, 8) == std::vector<std::string>{"aaa bbb ", "ccc ddd"});

    // Every byte of the line is on exactly one row, and nothing was inserted
    // between them — the line itself is untouched.
    std::string joined;
    for (const auto& row : layoutRows(bufferOf(lines, 0, 0), 8)) {
...
TEST_CASE("A word too long for the window is cut where the width falls [editlayout]") {
    // Cut a column short of the window like everything else: a word with
    // nowhere to break still leaves the rightmost column empty.
    const std::vector<std::string> lines{"aaaa bbbbbbbbbb"};
    CHECK(shownRows(lines, 7) == std::vector<std::string>{"aaaa ", "bbbbbb", "bbbb"});
}
...
TEST_CASE("The cursor stands on the row its character is drawn on [editlayout]") {
    const std::vector<std::string> lines{"aaa bbb ccc ddd"};
    const auto rows = layoutRows(bufferOf(lines, 0, 0), 8);
    REQUIRE(rows.size() == 2);
...
TEST_CASE("The rightmost column carries the cursor and never a character "
          "[editlayout]") {
    // Eleven characters in a window eleven wide: the eleventh column is kept
    // for the cursor, so the last word comes down onto a row of its own.
    ...
    // And it does so wherever the cursor stands: a line broken against that
    // would rewrap under the arrow keys.
    CHECK(layoutRows(bufferOf(lines, 0, 4), 11).size() == 2);

    // A column wider and the whole line fits, the cursor standing past the end
    // of it in the column kept for it.
    CHECK(layoutRows(bufferOf(lines, 0, lines[0].size()), 12).size() == 1);
}

TEST_CASE("A cursor at the end of a full row stands in the column kept for it "
          "[editlayout]") {
    // Three characters of text in a window four wide, the cursor past the end
    // of them: it stands in the fourth column, which is what that column is
    // for. Nothing is opened under the line for it.
    const std::vector<std::string> lines{"aaa"};
    const auto rows = layoutRows(bufferOf(lines, 0, 3), 4);
    REQUIRE(rows.size() == 1);
    CHECK(rows[0].end == 3);
    CHECK(rowOfCursor(rows, 0, 3) == 0);
}

TEST_CASE("A letter typed against the kept column takes its word down "
          "[editlayout]") {
    // "aaa bb" fills the six columns of text a window seven wide has, and the
    // cursor stands in the seventh. The letter typed there does not go into
    // that column: "bbb" no longer fits beside "aaa ", so the whole word comes
    // down onto the row below and the cursor with it.
    TextBuffer buffer = bufferOf({"aaa bb"}, 0, 6);
    CHECK(layoutRows(buffer, 7).size() == 1);

    insertText(buffer, "b", EditOptions{});
    const auto rows = layoutRows(buffer, 7);
    REQUIRE(rows.size() == 2);
    const std::string& line = buffer.line();
    CHECK(line.substr(rows[0].begin, rows[0].end - rows[0].begin) == "aaa ");
    CHECK(line.substr(rows[1].begin, rows[1].end - rows[1].begin) == "bbb");
...
TEST_CASE("Up and down move by rows of the screen, not by lines [editlayout]") {
    ...
    moveByRows(buffer, rows, -5, 8);
    CHECK(buffer.row == 0);
    CHECK(buffer.col == 2);
    moveByRows(buffer, rows, 9, 8);
    CHECK(buffer.row == 1);
}
...
TEST_CASE("Coming down onto a row the line goes on past stops inside it "
    ...
    const auto rows = layoutRows(bufferOf(lines, 0, 0), 5);
    REQUIRE(rows.size() == 4);
