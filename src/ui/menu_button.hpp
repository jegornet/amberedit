...
/// The way in, drawn in the top-right corner of the reader and of the editor: a
/// box two rows tall around the menu glyph, which clicking opens that screen's
/// context menu. `ui.menu_button` decides whether it is there at all; the
/// screens check that themselves, since where the two rows come from differs
/// between them.
///
...
///
/// The area list and the message list have no such corner: neither has a command
/// worth a menu yet.
...
/// The bottom row, which is what the second row of the screen costs: on both
/// screens it lands on the rule under the title. The rule stops a column short
/// of it, so the button reads as a thing standing beside it rather than a piece
/// of it.
