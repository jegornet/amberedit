...
Element render(FilePicker& picker, int inner) {
    ...
    for (int i = 0; i < picker.rows; ++i) {
        ...
        if (index == picker.cursor && !picker.onPath) {
            ...
            // The cursor is still on this row while the typing is up on the path
            // field, so the row says so quietly rather than wearing the fill of
            // a list being walked. `bold` and not `theme::selectionBold`: the
