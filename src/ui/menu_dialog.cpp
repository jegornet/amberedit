...
Element button(const config::AppConfig& config, const AppState::MenuView::Item& item,
               int inner, int icon, bool selected, bool pressed) {
    const Commands::Info& command = Commands::of(item.command);
    const std::string label =
        labelLine(command.icon, config.labelOf(item.command), inner - kIndent, icon);
...
Element render(AppState& state, Element background) {
    ...
    for (size_t i = 0; i < view.items.size(); ++i) {
        ...
        rows.push_back(hbox({text(side),
                             button(state.config, view.items[i], inner, icon,
