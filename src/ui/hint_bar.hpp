...
/// behind the key that runs it — `q reply  n reply-to  e new`. There is no help
...
/// than no row at all.
///
/// Which commands each screen offers is settled here and deliberately not
/// everything the screen answers: the row is a reminder of what there is to do,
/// and a list of every binding would be a page rather than a line. The message
/// list has none — every key on it moves the cursor — and its row is the rule
/// alone rather than given back, so that the screens on either side of it are
/// not a row taller.
...
/// the hints are a label in it. A screen with nothing to say leaves it whole.
///
